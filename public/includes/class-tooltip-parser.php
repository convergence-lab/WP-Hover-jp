<?php
require_once plugin_dir_path(__FILE__).'Shdev/FlashText/KeywordProcessor.php';
...
class WPGH_Tooltip_Parser
{
     ...
     * @var      string
     */
    protected $term_regex_pattern = '/(\b%s\b)(?![^<]*>|[^<>]*<\/)/u';
    ...
    public function parse_content($content)
    {
        ...
        foreach ($nodes as $node) {
            ...
            foreach ($terms->posts as $term) {
                ...
                // }
                if ($this->get_term_flashtext($term, $nodeValue) !== 1) {
                 continue;
                }
                ...
                $nodeValue    = $this->replace_term_flashtext($term, $tooltip, $nodeValue);
    ...
    private function get_term_flashtext($term, $nodeValue)
    {
      $keywords = [
      	$term->post_title => [$term->post_title]
      ];
      $keywordProcessor= new KeywordProcessor();
      $keywordProcessor->addKeywordsFromAssocArray($keywords);
      $keywordsExtracted = $keywordProcessor->extractKeywords($nodeValue, false);
      $cnt = count($keywordsExtracted);
      if ($cnt > 0) {
        return 1;
      } else {
        return 0;
    ...
    private function replace_term_flashtext($term, $tooltip, $nodeValue)
    {
      $keywords = [
        $tooltip => [$term->post_title]
      ];
      $keywordProcessor= new KeywordProcessor();
      $keywordProcessor->addKeywordsFromAssocArray($keywords);
      return $keywordProcessor->replaceKeywords($nodeValue);
    }
